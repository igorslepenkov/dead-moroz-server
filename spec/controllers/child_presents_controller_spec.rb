...
RSpec.describe Users::ChildPresentsController, type: :controller do
  ...
  describe 'POST create' do
    context 'if params are valid' do
      it 'adds 2 child_presents to user' do
        post :create, params: { child_presents: [{ name: 'IPhone' }, { name: 'Cat' }], id: 1 }
        expect(User.find(1).child_presents.count).to eq(2)
      end
    ...
    context 'if params are invalid' do
      it 'responses with message and errors hash' do
        post :create, params: { child_presents: [{ name: '' }, { name: '' }], id: 1 }
